...
export class ToolbarComponent extends LitElement {
  ...
  static styles = css`
    ...
    .dropdown-content {
      background: var(--hx-background-200);
      border: 1px solid var(--hx-border-100);
      border-radius: 8px;
      padding: 0.25rem;
      display: none;
    ...
    .dropdown-item:not(.menu-browse):not(.menu-maplabs):not(.menu-lego):not(
        .menu-software
      ):hover {
      background: color-mix(in srgb, #fff 5%, transparent);
    }
    ...
    .menu-browse {
      background: linear-gradient(
        6deg,
        var(--hx-text-brand) 0%,
        var(--hx-background-200) 50%,
        var(--hx-background-300) 100%
      );
      transition: all 0.4s ease;
    }

    .menu-browse:hover {
      border: 1px solid var(--hx-border-300) !important;
    }

    .menu-maplabs {
      background: linear-gradient(
        -12deg,
        var(--hx-collection-maplabs) 0%,
        var(--hx-background-200) 78%
      );
    }
  ...
  private getDropdownContentElement(
    dropdownType: string
  ): Element | null | undefined {
    if (!dropdownType) return null;
    // Dropdown triggers have IDs: "toolbar-resolution", "toolbar-aspectRatio", "toolbar-sort", "menu"
    const selector = `#${
      dropdownType.startsWith("toolbar-")
        ? dropdownType
        : dropdownType === "menu"
        ? "menu"
        : `toolbar-${dropdownType}`
    } .dropdown-content`;
    return this.shadowRoot?.querySelector(selector);
  }

  setActiveDropdown(newActiveType: string) {
    // newActiveType can be "resolution", "aspectRatio", "sortBy", "menu", or ""
    ...
    if (newActiveType) {
      const targetDropdownContent =
        this.getDropdownContentElement(newActiveType);
    ...
    if (previouslyActiveType && previouslyActiveType !== newActiveType) {
      const previouslyActiveDropdownContent =
        this.getDropdownContentElement(previouslyActiveType);
  ...
  handleDropdownMouseLeave(dropdownType: string) {
    ...
    if (this.activeDropdown === dropdownType) {
      const dropdownContentElement =
        this.getDropdownContentElement(dropdownType);

      if (
        !dropdownContentElement ||
        dropdownContentElement.classList.contains("closing")
      ) {
      ...
      dropdownContentElement.addEventListener(
        "animationend",
        animationEndHandler,
        { once: true }
      );
  ...
  handleCheckboxSelect(type: "resolution" | "aspectRatio", value: string) {
    ...
    window.dispatchEvent(
      new CustomEvent("vault-filter-change", {
